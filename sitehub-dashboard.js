(()=>{
  ...
  function renderAnalytics(){
    ...
    root.innerHTML=`
      ...
      ${periodControls()}
      <div class="obs-metrics">${metric('오늘 PV',num(t.today_pv),`${delta} · 정제값`,'','pv')}${metric('오늘 고유 브라우저 (UV)',num(t.today_uv),'사람 수가 아닌 익명 브라우저 ID 기준','','uv')}${metric('최근 7일 PV',num(t.pv_7d),`고유 브라우저 ${num(t.uv_7d)} · 정제값`,'','pv')}${metric('최근 30일 PV',num(t.pv_30d),`고유 브라우저 ${num(t.uv_30d)} · 정제값`,'','pv')}${metric('정상 수집',`${num(app.analyticsNormal)} / ${num(app.webTotal)}`,`미설치 ${num(app.analyticsMissing)} · 장기미수신 ${num(app.analyticsStale)} · 해당 없음 ${num(app.notApplicable)}`,app.analyticsIssue?'attention':'','analytics')}${metric('SiteHub 적용',`${num(tot.sitehub_connected)} / ${num(tot.registry_projects)}`,`미적용 ${num(tot.sitehub_missing)}`,tot.sitehub_missing?'attention':'') }</div>
  ...
  function renderRankedProjects(rows){if(!rows.length)return '<p class="obs-empty">집계 데이터가 없습니다.</p>';const max=Math.max(1,...rows.map(x=>Number(x.pv||0)));return `<div class="obs-ranking">${rows.slice(0,12).map((p,i)=>`<div class="obs-ranking-row ${state.selectedProjectId===p.project_id?'active':''}"><button class="obs-ranking-main" data-obs-chart-project="${E(p.project_id)}" aria-label="${E(p.name||p.project_id)} 그래프로 보기"><em>${i+1}</em><span><b>${E(p.name||p.project_id)}</b><small>${E(p.project_id)} · 고유 브라우저 ${num(p.uv)} · 클릭하면 위 그래프 전환</small><i style="--bar:${Math.max(3,Number(p.pv||0)/max*100)}%"></i></span><strong>${num(p.pv)}<small>PV</small></strong></button><button class="obs-detail-button" data-obs-detail-project="${E(p.project_id)}" aria-label="${E(p.name||p.project_id)} 상세 보기">상세</button></div>`).join('')}</div>`}
  ...
  function renderSearch(){
    ...
    root.innerHTML=`
      ...
      <article class="obs-card obs-search-card">
        ...
        <div class="obs-search-table-wrap"><table class="obs-search-table"><thead><tr><th>프로젝트</th><th>${term('검색 기본','seo')}</th><th>${term('답변형 검색','aeo')}</th><th>${term('생성형 검색','geo')}</th><th>${term('검색 준비도','readiness')}</th><th>${term('영상 검색','videoseo')}</th><th>${term('Google 색인','googleindex')}</th><th>${term('Sitemap','sitemap')}</th><th>${term('Robots','robots')}</th><th>${term('페이지 기본정보','namecard')}</th><th>최근 점검</th></tr></thead><tbody id="obsSearchRows"></tbody></table></div>
      </article>`;
