(()=>{
  ...
  function renderAnalytics(){
    ...
    root.innerHTML=`
      ...
      ${periodControls()}
      <div class="obs-metrics">${metric('오늘 PV',num(t.today_pv),delta)}${metric('오늘 UV',num(t.today_uv),'익명 visitor 기준')}${metric('최근 7일 PV',num(t.pv_7d),`UV ${num(t.uv_7d)}`)}${metric('최근 30일 PV',num(t.pv_30d),`UV ${num(t.uv_30d)}`)}${metric('정상 수집',`${num(app.analyticsNormal)} / ${num(app.webTotal)}`,`미설치 ${num(app.analyticsMissing)} · 장기미수신 ${num(app.analyticsStale)} · 해당 없음 ${num(app.notApplicable)}`,app.analyticsIssue?'attention':'')}${metric('SiteHub 적용',`${num(tot.sitehub_connected)} / ${num(tot.registry_projects)}`,`미적용 ${num(tot.sitehub_missing)}`,tot.sitehub_missing?'attention':'') }</div>
      ${renderTrend(t.trend||[])}
      <div class="obs-two-col">
        ...
        <article class="obs-card"><div class="obs-card-head"><div><span class="metric-kicker">COLLECTION</span><h3>수집 상태</h3></div><small>${num(attention.length)}개 확인 필요</small></div><div class="obs-status-list">${receiving.slice(0,6).map(p=>statusLine(p,'normal')).join('')}${attention.slice(0,10).map(p=>statusLine(p,p.analytics_status)).join('')||'<p class="obs-empty">확인 필요한 프로젝트가 없습니다.</p>'}</div></article>
      </div>
      <div class="obs-two-col">
  ...
  function renderProjectDialog(d){
    ...
    body.innerHTML=`<div class="modal-head"><div><p class="eyebrow">${E(p.project_id)} · SITEHUB</p><h2>${E(p.name)}</h2><p class="obs-dialog-url">${p.public_url?`<a href="${E(p.public_url)}" target="_blank" rel="noreferrer">${E(p.public_url)}</a>`:'운영 URL 없음'}</p></div><button class="icon-button" data-obs-close aria-label="닫기">×</button></div>
      <div class="obs-detail-strip"><span><small>서비스</small>${chip(p.service_status)}</span><span><small>NameCard</small>${chip(ncStatus)}</span><span><small>Analytics</small>${chip(analyticsStatus)}</span><span><small>Search readiness</small>${chip(s.readiness||'not_checked')}</span><span><small>VideoSEO</small>${chip(video.state,video.label)}</span></div>
      <div class="obs-dialog-grid"><section><h3>Analytics · 30일</h3><div class="obs-dialog-metrics"><span><small>PV</small><strong>${num(p.pv_30d)}</strong></span><span><small>UV</small><strong>${num(p.uv_30d)}</strong></span><span><small>마지막 이벤트</small><strong>${!applicable?'해당 없음':p.last_event_at?ago(p.last_event_at):'미수신'}</strong></span></div>${renderPages(t.top_pages||[])}${renderReferrers(t.top_referrers||[])}</section>
      <section><h3>SearchOps 근거</h3>${applicable?searchEvidence(s,idx,p)+videoSEOEvidence(s,p):'<p class="obs-empty">웹 운영 URL이 없어 SearchOps 적용 대상이 아닙니다.</p>'}</section></div>
  ...
  function videoSEOEvidence(s,p){
    ...
    return `<div class="obs-index-box"><strong>VideoSEO</strong><p>watch pages ${numMaybe(v.watch_pages)} · ACTIVE ${numMaybe(st.active)} · TRENDING ${numMaybe(st.trending)} · ARCHIVED ${numMaybe(st.archived)} · SOURCE_UNAVAILABLE ${numMaybe(st.source_unavailable)} · REMOVED ${numMaybe(st.removed)}</p></div><div class="obs-evidence"><div><span>적용</span>${chip(vs.state,vs.label)}</div><div><span>VideoObject</span>${chip(vo.status||'not_checked')}</div><div><span>Video sitemap</span>${chip(sm.status||'not_checked')}</div><div><span>Watch URL 파손</span><strong>${numMaybe(wu.broken)}</strong></div><div><span>404 / 410</span><strong>${numMaybe(wu.http_404)} / ${numMaybe(wu.http_410)}</strong></div><div><span>Source unavailable</span><strong>${numMaybe(su.count)} · ${su.ratio===null||su.ratio===undefined?'-':pct(su.ratio)}</strong></div></div>${sitemapText}<div class="obs-index-box"><strong>Video Search Console</strong><p>${scReady?`Indexed watch ${numMaybe(sc.indexed_watch_pages)} · Video indexed ${numMaybe(sc.video_indexed)} · failed ${numMaybe(sc.video_failed)} · Rich result 오류 ${numMaybe(sc.rich_result_errors)} · 노출 ${numMaybe(sc.impressions)} · 클릭 ${numMaybe(sc.clicks)}`:'검증된 Search Console Video 지표가 아직 연결되지 않아 수집 대기입니다.'}</p></div>${alertText}`;
  }
