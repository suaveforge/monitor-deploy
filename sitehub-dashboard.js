(()=>{
  ...
  function renderSearch(){
    const d=state.data,tot=d.totals||{},ps=d.projects||[];const app=obsApplicability(tot,ps);const root=q('#sitehubObservability');if(!root)return;
    root.innerHTML=`
  ...
  function renderProjectDialog(d){
    const p=d.projects?.[0];if(!p)return;const body=q('#obsProjectDialogBody'),t=d.traffic||{},s=p.search||{},idx=s.google_index||{};const applicable=obsWebApplicable(p),ncStatus=applicable?(p.namecard_status==='normal'?'pass':'fail'):'not_applicable',analyticsStatus=applicable?(p.analytics_status||'unknown'):'not_applicable',searchStatus=applicable?(s.readiness||'not_checked'):'not_applicable',rawErrors=[...(s.errors||[]),...(p.alerts||[])],errors=applicable?rawErrors:rawErrors.filter(x=>{const v=String(typeof x==='string'?x:'');return v!=='namecard_fail'&&!v.startsWith('analytics_')&&!v.startsWith('search_')&&v!=='sitemap_fail'&&v!=='robots_fail'});
    body.innerHTML=`<div class="modal-head"><div><p class="eyebrow">${E(p.project_id)} · SITEHUB</p><h2>${E(p.name)}</h2><p class="obs-dialog-url">${p.public_url?`<a href="${E(p.public_url)}" target="_blank" rel="noreferrer">${E(p.public_url)}</a>`:'운영 URL 없음'}</p></div><button class="icon-button" data-obs-close aria-label="닫기">×</button></div>
      <div class="obs-detail-strip"><span><small>서비스</small>${chip(p.service_status)}</span><span><small>NameCard</small>${chip(ncStatus)}</span><span><small>Analytics</small>${chip(analyticsStatus)}</span><span><small>Search readiness</small>${chip(s.readiness||'not_checked')}</span></div>
      ...
      <section><h3>SearchOps 근거</h3>${applicable?searchEvidence(s,idx,p):'<p class="obs-empty">웹 운영 URL이 없어 SearchOps 적용 대상이 아닙니다.</p>'}</section></div>
