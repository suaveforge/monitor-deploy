(()=>{
  ...
  function renderSearch(){
    const d=state.data,tot=d.totals||{},ps=d.projects||[];const app=obsApplicability(tot,ps),video=videoSEOSummary(app.webProjects);const root=q('#sitehubObservability');if(!root)return;
    root.innerHTML=`
      ...
      <div class="obs-metrics search-metrics">${metric('검색 기본 점검 (SEO)',`${num(tot.seo_pass)} / ${num(app.webTotal)}`,`FAIL ${num(tot.seo_fail)}`,'','seo')}${metric('답변형 검색 점검 (AEO)',`${num(tot.aeo_pass)} / ${num(app.webTotal)}`,`FAIL ${num(tot.aeo_fail)}`,'','aeo')}${metric('생성형 검색 점검 (GEO)',`${num(tot.geo_pass)} / ${num(app.webTotal)}`,`FAIL ${num(tot.geo_fail)}`,'','geo')}${metric('검색 준비도',`${num(tot.readiness_pass)} / ${num(app.webTotal)}`,`FAIL ${num(tot.readiness_fail)}`,'','readiness')}${metric('Sitemap',`${num(tot.sitemap_pass)} / ${num(app.webTotal)}`,`FAIL ${num(tot.sitemap_fail)}`,'','sitemap')}${metric('Google Index',`${num(app.indexVerified)} 확인됨`,`${num(app.indexUnknown)} unknown · 해당 없음 ${num(app.notApplicable)}`,'','googleindex')}${metric('VideoSEO',`${num(video.provided)} 제공`, `수집 대기 ${num(video.pending)} · 미지원 ${num(video.unsupported)} · 해당 없음 ${num(video.notApplicable)} · 경보 ${num(video.alertProjects)}`,video.alertProjects?'attention':'','videoseo')}</div>
      <article class="obs-card obs-search-card">
        <div class="obs-card-head"><div><span class="metric-kicker">REGISTRY WIDE</span><h3>전 프로젝트 SearchOps</h3></div><div class="obs-search-filter"><input id="obsSearchFilter" type="search" placeholder="프로젝트·도메인 검색"><select id="obsSearchState"><option value="all">전체 상태</option><option value="fail">FAIL 포함</option><option value="pass">Readiness PASS</option><option value="video">VideoSEO 경보</option><option value="unknown">미점검 / Index unknown</option></select></div></div>
        <div class="obs-search-table-wrap"><table class="obs-search-table"><thead><tr><th>Project</th><th>SEO</th><th>AEO</th><th>GEO</th><th>Readiness</th><th>VideoSEO</th><th>Index</th><th>Sitemap</th><th>Robots</th><th>NameCard</th><th>Last check</th></tr></thead><tbody id="obsSearchRows"></tbody></table></div>
      </article>`;
  ...
  function renderProjectDialog(d){
    ...
    body.innerHTML=`<div class="modal-head"><div><p class="eyebrow">${E(p.project_id)} · SITEHUB</p><h2>${E(p.name)}</h2><p class="obs-dialog-url">${p.public_url?`<a href="${E(p.public_url)}" target="_blank" rel="noreferrer">${E(p.public_url)}</a>`:'운영 URL 없음'}</p></div><button class="icon-button" data-obs-close aria-label="닫기">×</button></div>
      <div class="obs-detail-strip"><span><small>서비스 상태</small>${chip(p.service_status)}</span><span><small>${term('페이지 기본정보','namecard')}</small>${chip(ncStatus)}</span><span><small>${term('방문 수집','analytics')}</small>${chip(analyticsStatus)}</span><span><small>${term('검색 준비도','readiness')}</small>${chip(s.readiness||'not_checked')}</span><span><small>${term('영상 검색','videoseo')}</small>${chip(video.state,video.label)}</span></div>
      <div class="obs-dialog-grid"><section><h3>방문 분석 · 최근 30일</h3><div class="obs-dialog-metrics"><span><small>${term('PV','pv')}</small><strong>${num(p.pv_30d)}</strong></span><span><small>${term('UV','uv')}</small><strong>${num(p.uv_30d)}</strong></span><span><small>마지막 수집</small><strong>${!applicable?'해당 없음':validDate(p.last_event_at)?ago(p.last_event_at):'수집 기록 없음'}</strong></span></div>${renderProjectVisitTrend(t.trend||[])}<div class="obs-detail-subhead">상위 페이지</div>${renderPages(t.top_pages||[])}<div class="obs-detail-subhead">주요 유입 경로</div>${renderReferrers(t.top_referrers||[])}</section>
  ...
  function renderProjectVisitTrend(rows){
    if(!rows.length)return '<div class="obs-mini-trend empty"><strong>방문 추이</strong><p>일별 PV/UV 데이터가 아직 없습니다.</p></div>';
    const w=620,h=150,pad=18,max=Math.max(1,...rows.flatMap(x=>[Number(x.pv||0),Number(x.uv||0)]));
    return `<div class="obs-mini-trend"><div class="obs-mini-trend-head"><strong>일별 방문 추이</strong><span>${term('PV','pv')} · ${term('UV','uv')} · ${E(trendDelta(rows,'pv'))}</span></div><svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" aria-label="프로젝트 일별 PV와 UV 추이"><polyline class="pv" points="${linePoints(rows,'pv',max,w,h,pad)}"></polyline><polyline class="uv" points="${linePoints(rows,'uv',max,w,h,pad)}"></polyline></svg><div class="obs-chart-axis"><span>${E(dayLabel(rows[0]?.day||''))}</span><strong>최고 ${num(max)}</strong><span>${E(dayLabel(rows[rows.length-1]?.day||''))}</span></div></div>`;
  }
  ...
  function videoSEOEvidence(s,p){
    ...
    return `<div class="obs-index-box"><strong>${term('VideoSEO 상태','videoseo')}</strong><p>${term('감시 영상 페이지','watchpages')} ${numMaybe(v.watch_pages)} · ${term('정상 공개','active')} ${numMaybe(st.active)} · ${term('추세 감지','trending')} ${numMaybe(st.trending)} · ${term('보관','archived')} ${numMaybe(st.archived)} · ${term('원본 확인 불가','sourceunavailable')} ${numMaybe(st.source_unavailable)} · ${term('제거','removed')} ${numMaybe(st.removed)}</p></div><div class="obs-evidence"><div><span>적용 상태</span>${chip(vs.state,vs.label)}</div><div><span>${term('영상 구조화 데이터','videoobject')}</span>${chip(vo.status||'not_checked')}</div><div><span>${term('영상 Sitemap','sitemap')}</span>${chip(sm.status||'not_checked')}</div><div><span>영상 페이지 URL 오류</span><strong>${numMaybe(wu.broken)}</strong></div><div><span>404 / 410</span><strong>${numMaybe(wu.http_404)} / ${numMaybe(wu.http_410)}</strong></div><div><span>${term('원본 확인 불가','sourceunavailable')}</span><strong>${numMaybe(su.count)} · ${su.ratio===null||su.ratio===undefined?'-':pct(su.ratio)}</strong></div></div>${sitemapText}<div class="obs-index-box"><strong>Video Search Console</strong><p>${scReady?`색인 감시 페이지 ${numMaybe(sc.indexed_watch_pages)} · 영상 색인 ${numMaybe(sc.video_indexed)} · 실패 ${numMaybe(sc.video_failed)} · Rich result 오류 ${numMaybe(sc.rich_result_errors)} · 검색 노출 ${numMaybe(sc.impressions)} · 클릭 ${numMaybe(sc.clicks)}`:'검증된 Search Console Video 지표가 아직 연결되지 않아 수집 대기입니다.'}</p></div>${alertText}`;
  }
  function searchEvidence(s,idx,p){
    if(!s||!Object.keys(s).length)return '<p class="obs-empty">SearchOps 점검 결과가 아직 없습니다.</p>';
    const rows=[['검색 기본 점검 (SEO)','seo',s.seo_status],['답변형 검색 점검 (AEO)','aeo',s.aeo_status],['생성형 검색 점검 (GEO)','geo',s.geo_status],['검색 기술 준비도','readiness',s.technical_readiness||s.readiness],['대표 URL 선언','canonical',s.root?.canonical_ok?'pass':'fail'],['Googlebot 콘텐츠 동일성','googlebot',s.root?.googlebot_parity_ok?'pass':'fail'],['Sitemap','sitemap',s.sitemap?.status],['Robots','robots',s.robots?.status],['구조화 데이터','structured',s.structured_data?.valid?'pass':s.structured_data?.present?'warn':'fail'],['내부 링크','internal',Number(s.crawl?.internal_links_broken||0)>0?'fail':'pass']];
    return `<div class="obs-evidence">${rows.map(([label,key,v])=>`<div><span>${term(label,key)}</span>${chip(v||'not_checked')}</div>`).join('')}</div><div class="obs-index-box"><strong>${term('Google Index','googleindex')}</strong><p>${idx.status==='unknown'?'검증된 Google Index 데이터 소스가 아직 연결되지 않았습니다. 검색 기술 준비도와 실제 Google 색인은 별개라 Unknown으로 표시합니다.':`색인됨 ${num(idx.indexed)} · 발견됨 ${num(idx.discovered)}`}</p></div>${s.sitemap?.url?`<p class="obs-evidence-url"><b>${term('Sitemap','sitemap')}</b> ${E(s.sitemap.url)} ${s.sitemap.http_status?`· HTTP ${E(s.sitemap.http_status)}`:''}</p>`:''}${s.robots?.url?`<p class="obs-evidence-url"><b>${term('Robots','robots')}</b> ${E(s.robots.url)} ${s.robots.http_status?`· HTTP ${E(s.robots.http_status)}`:''}</p>`:''}${p.namecard_missing?.length?`<p class="obs-evidence-url"><b>${term('페이지 기본정보 누락','namecard')}</b> ${E(p.namecard_missing.join(', '))}</p>`:''}`
  }
